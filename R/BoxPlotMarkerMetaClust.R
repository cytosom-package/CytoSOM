...
#' @param treatmentTable  data frame containing a column 'files' and a column 'Treatment' (case sensitive, avoid special character, especially "-")
...
#' @param ExportData if TRUE, export MFI,Median of MFI, pairwise comparison p-values, controle vs treatment p-values to csv files
#' @return list containing MFI,Median of MFI, pairwise comparison p-values, controle vs treatment p-values
...
BoxPlotMarkerMetaClust = function(TreeMetaCl,Title,treatmentTable,
                                  ControlTreatment,BottomMargin,Marker,Robust=TRUE,ClustHeat=TRUE,ExportData=FALSE) {
    ...
    BoxPlotMetaClustFull(TreeMetaCl,Title,treatmentTable,ControlTreatment,
                         BottomMargin,yLab="",Norm=FALSE,Marker,Robust,ClustHeat,ExportData) }
