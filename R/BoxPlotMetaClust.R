...
#' @param treatmentTable  data frame containing a column 'files',a column 'Treatment' (case sensitive, avoid special character, especially "-") and a column "NormalizationFactor" for the optional normalization of the data
...
#' @param ExportData if TRUE, export sizes,median of sizes, pairwise comparison p-values, controle vs treatment p-values to csv files
#' @return list containing sizes,median of sizes, pairwise comparison p-values, controle vs treatment p-values
...
#' @export
BoxPlotMetaClust = function(TreeMetaCl,Title,treatmentTable,ControlTreatment,BottomMargin,yLab,Norm=FALSE,Robust = TRUE,ClustHeat=TRUE,ExportData=FALSE) {
  if (Norm & (!is.numeric(treatmentTable$NormalizationFactor))) {stop("non-numeric normalization factor in table")}
   BoxPlotMetaClustFull(TreeMetaCl,Title,treatmentTable,ControlTreatment,BottomMargin,yLab,Norm,Marker="",Robust,ClustHeat,ExportData)
}
